# Cours : Web Sémantique et Ontologies pour le Patrimoine Culturel
...
2. [Architecture du web et fondamentaux](#architecture)
3. [Principes du web sémantique](#principes)
4. [RDF : Le modèle de données](#rdf)
5. [Vocabulaires et ontologies patrimoniales](#vocabulaires)
6. [Applications pratiques](#applications)
7. [Travaux pratiques](#tp)
...
### 1.1 
### 1.2 Les enjeux pour les institutions patrimoniales

**Interopérabilité** : Vos données peuvent dialoguer avec celles d'autres musées, archives, bibliothèques sans avoir à tout refaire.

**Découvrabilité** : Les usagers peuvent trouver vos œuvres via des recherches transversales inimaginables auparavant.

**Enrichissement mutuel** : En publiant vos données structurées, vous bénéficiez automatiquement des connaissances produites ailleurs.

**Mais aussi des défis :**
- Changement d'échelle et perte potentielle de contrôle sur "son" catalogue
- Nécessité de repenser l'autorité institutionnelle dans un contexte distribué
- Questions sur l'ouverture des données et les licences

---

---

<a name="vocabulaires"></a>
## 5. Vocabulaires et ontologies patrimoniales

### 5.1 
### 5.2 

### 5.3 Autres ontologies et vocabulaires importants

**Dublin Core** : Métadonnées simples et largement adoptées
- `dc:title`, `dc:creator`, `dc:date`, `dc:subject`
- Très utilisé mais peu expressif

**EDM (Europeana Data Model)** : Pour l'agrégation
- Distingue l'objet physique de ses représentations numériques
- Permet de conserver les métadonnées originales tout en normalisant

**AAT (Art & Architecture Thesaurus)** : Vocabulaire du Getty
- Matériaux : `aat:300011727` (huile)
- Techniques : `aat:300054216` (peinture sur toile)
- Styles : `aat:300021147` (baroque)
- Fonctions : `aat:300263552` (portrait)

**SKOS** : Pour structurer des thésaurus
- `skos:Concept` : Un concept
- `skos:prefLabel` : Terme préféré
- `skos:altLabel` : Synonyme
- `skos:broader` / `skos:narrower` : Relations hiérarchiques
- `skos:related` : Termes associés

**Linked Art** : Ontologie récente basée sur CIDOC-CRM
- Profil d'application simplifié de CIDOC-CRM
- Patterns documentés pour cas d'usage courants
- Format JSON-LD privilégié
- Adopté par plusieurs musées américains

### 5.4 RDF Schema et OWL : Construire des ontologies

**RDFS (RDF Schema)** permet de :
- Définir des classes : `rdfs:Class`
- Créer des hiérarchies : `rdfs:subClassOf`
- Définir des propriétés : `rdf:Property`
- Spécifier domaines et portées : `rdfs:domain`, `rdfs:range`

**OWL (Web Ontology Language)** ajoute :
- Logique plus riche (équivalence, disjonction)
- Restrictions de cardinalité
- Propriétés transitives, symétriques, inverses
- Raisonnement automatique

**En pratique pour vous :**
Vous n'aurez probablement pas à créer une ontologie complète, mais plutôt à :
1. Réutiliser des ontologies existantes (CIDOC-CRM, Dublin Core)
2. Les étendre si nécessaire pour vos besoins spécifiques
3. Aligner vos termes locaux avec des vocabulaires standards
...
**Scénario 1 : Tracer la provenance**
Une sculpture a appartenu à 5 collectionneurs différents avant d'entrer au musée. Avec CIDOC-CRM, vous modélisez chaque transfert comme un événement `E8_Acquisition` avec :
- Les acteurs impliqués (vendeur, acheteur)
- La date
- Le lieu
- Le prix (si connu)

**Scénario 2 : Documenter les restaurations**
Votre tableau a subi 3 restaurations. Chaque intervention est un événement `E11_Modification` :
- Techniques utilisées (vernissage, repeint)
- Intervenants (restaurateurs)
- Impact sur l'œuvre
- Documentation photographique

**Scénario 3 : Interconnexion avec d'autres musées**
Vous avez un portrait de Napoléon. En utilisant l'URI Wikidata de Napoléon, vous créez automatiquement des liens avec :
- Les autres portraits de Napoléon dans d'autres musées
- Sa biographie sur Wikipédia
- Les lieux associés à sa vie (Getty TGN)
- Les événements historiques

### 6.2 
### 6.3 Bénéfices concrets

**Pour l'institution :**
- Visibilité accrue (SEO, découvrabilité)
- Économies d'échelle (réutilisation de référentiels)
- Qualité améliorée par enrichissement mutuel

**Pour les chercheurs :**
- Recherches trans-institutionnelles
- Analyses à grande échelle
- Nouvelles questions de recherche possibles

**Pour le public :**
- Découvertes inattendues (serendipity)
- Contexte enrichi automatiquement
- Expériences multilingues facilitées
...
Modéliser une œuvre de votre collection en utilisant les principes du web sémantique, l'ontologie CIDOC-CRM et les vocabulaires appropriés.

---

## TP : Modélisation d'une œuvre patrimoniale en Linked Data
...
1. Identifier les entités et relations pertinentes pour décrire une œuvre patrimoniale
2. Appliquer les classes et propriétés de CIDOC-CRM
3. Créer un graphe conceptuel représentant les données
4. Encoder ces données en RDF/Turtle
5. Réfléchir de manière critique sur les avantages et limites du web sémantique

---

### Partie 1 : Choix et analyse d'une œuvre (30 min)

#### 1.1 Sélection de l'œuvre

Choisissez une œuvre patrimoniale (réelle ou fictive) qui présente une **richesse documentaire**. Privilégiez un objet dont vous connaissez :
- L'histoire (création, acquisitions, restaurations)
- Le contexte (qui, quand, où, pourquoi)
- Les matériaux et techniques

**Exemples recommandés :**
- Une peinture avec une provenance documentée
- Une sculpture ayant subi des restaurations
- Un objet ethnographique avec un contexte de collecte connu
- Un meuble avec des changements de propriétaires multiples

#### 1.2 Collecte d'informations

Documentez **au minimum** les éléments suivants :

**Informations de base :**
- Titre de l'œuvre
- Description (en 2-3 phrases)
- Type d'objet (peinture, sculpture, textile, etc.)
- Numéro d'inventaire

**Création :**
- Créateur(s) : nom complet, dates de vie si connues
- Date de création (précise ou approximative)
- Lieu de création (ville, pays)
- Matériaux utilisés
- Techniques employées
- Dimensions (hauteur, largeur, profondeur)

**Événements dans la vie de l'objet (au moins 2) :**
- Acquisition par votre institution (date, mode d'acquisition, provenance)
- ET au moins un parmi :
  - Restauration (date, nature de l'intervention, restaurateur)
  - Exposition (titre, lieu, dates)
  - Prêt à une autre institution
  - Changement d'attribution
  - Transfert de propriété antérieur

**Localisation actuelle :**
- Département/Collection
- Salle d'exposition ou réserve
- Conditions de conservation

#### 1.3 Organisation de vos données

Créez un document (Word/Google Doc/notes) structuré ainsi :

```
ŒUVRE : [Titre]
Numéro d'inventaire : [XXX]

CRÉATION
- Créateur : [Nom] (dates de vie)
...
ÉVÉNEMENT 1 : [Type d'événement]
- Date : [préciser]
- Lieu : [si pertinent]
- Acteurs impliqués : [noms]
- Description : [détails]

ÉVÉNEMENT 2 : [Type d'événement]
- Date : [préciser]
- Lieu : [si pertinent]
- Acteurs impliqués : [noms]
- Description : [détails]
...
LOCALISATION ACTUELLE
- Institution : [nom complet]
- Département : [nom]
- Emplacement : [salle ou réserve]
```

**💡 Conseil :** Si vous manquez d'informations, vous pouvez inventer des détails plausibles pour l'exercice (mais indiquez-le clairement).

---

### Partie 2 : Modélisation conceptuelle (45 min)

#### 2.1 Identifier les classes CIDOC-CRM appropriées

Pour chaque élément de votre documentation, identifiez la **classe CIDOC-CRM** correspondante.

**Guide de correspondance :**

| Ce que vous documentez | Classe CIDOC-CRM | Code |
|------------------------|------------------|------|
| L'œuvre elle-même | Human-Made Object | E22 |
| Le créateur (artiste, artisan) | Person | E21 |
| L'événement de création | Production | E12 |
| Un lieu (atelier, ville) | Place | E53 |
| Une période temporelle | Time-Span | E52 |
| Un matériau (toile, bronze) | Material | E57 |
| Une technique (peinture à l'huile) | Type | E55 |
| Votre musée (institution) | Group | E74 |
| Un événement d'acquisition | Acquisition | E8 |
| Une restauration | Modification | E11 |
| Une exposition | Activity | E7 |
| Un titre/appellation | Appellation | E41 |
| Une dimension (hauteur, poids) | Dimension | E54 |

**Exemple :**
Si vous avez : "Portrait de Marie-Antoinette, huile sur toile, 1783, par Antoine Vestier"

Vous identifiez :
- Portrait de Marie-Antoinette → E22_Human-Made_Object
- Antoine Vestier → E21_Person
- L'acte de peindre ce portrait → E12_Production
- 1783 → E52_Time-Span
- Huile → E57_Material
- Toile → E57_Material
- Peinture à l'huile → E55_Type (technique)

#### 2.2 Créer le graphe conceptuel

**Outil recommandé :** Papier et crayon OU un outil de dessin simple (PowerPoint, Draw.io, Excalidraw)

**Instructions :**

1. **Représentez les entités comme des rectangles ou des cercles**
   - Écrivez le nom de la ressource
   - Indiquez la classe CIDOC-CRM entre parenthèses
   - Exemple : `[Portrait de Marie-Antoinette (E22)]`

2. **Représentez les relations comme des flèches**
   - La flèche part du sujet vers l'objet
   - Étiquetez la flèche avec la propriété CIDOC-CRM
   - Exemple : `[Œuvre] --P108_was_produced_by--> [Production]`

3. **Ajoutez les littéraux en italique ou entre guillemets**
   - Exemple : `"1783"`, `"Antoine Vestier"@fr`

**Structure minimale à représenter :**

```
[Œuvre: E22]
    |--P102_has_title--> "Titre de l'œuvre"@fr
    |--P108_was_produced_by--> [Production: E12]
    |                             |--P14_carried_out_by--> [Artiste: E21]
    |                             |--P4_has_time-span--> [Période: E52]
    |                             |--P7_took_place_at--> [Lieu: E53]
    |--P45_consists_of--> [Matériau: E57]
    |--P32_used_general_technique--> [Technique: E55]
    |--P52_has_current_owner--> [Musée: E74]
    |--P24i_changed_ownership_through--> [Acquisition: E8]
    |--P43_has_dimension--> [Dimension: E54]
```

**Pour vos événements supplémentaires** (restauration, exposition, etc.), ajoutez de nouvelles branches suivant le même principe.

#### 2.3 Enrichir avec des vocabulaires externes

Identifiez où vous pourriez utiliser des **URI externes** au lieu de simples chaînes de caractères :

**AAT (Getty) pour les matériaux et techniques :**
- Huile → `aat:300015050`
- Toile → `aat:300014078`
- Peinture (technique) → `aat:300054216`
- Bronze → `aat:300010957`

**Wikidata pour les personnes et lieux :**
- Marie-Antoinette → `wd:Q47365`
- Paris → `wd:Q90`
- Antoine Vestier → `wd:Q2853811`

**Getty TGN pour les lieux géographiques :**
- Paris → `tgn:7008038`
- Versailles → `tgn:7008387`

**Dans votre graphe, annotez ces enrichissements :**
Exemple : `[Huile: E57] owl:sameAs <aat:300015050>`

---

### Partie 3 : Encodage en RDF/Turtle (45 min)

#### 3.1 Structure de base du fichier

Créez un fichier texte (extension `.ttl`) et commencez par déclarer vos **préfixes** :
...
@prefix votremusee: <http://votremusee.org/> .
```

**💡 Conseil :** Inventez un domaine pour votre musée (ex: `http://museedemonreal.ca/`)

#### 3.2 Encoder l'œuvre elle-même

```turtle
votremusee:objet/001 a crm:E22_Human-Made_Object ;
    rdfs:label "Titre de votre œuvre"@fr ;
    crm:P102_has_title "Titre de votre œuvre"@fr ;
    crm:P108_was_produced_by votremusee:production/001 ;
    crm:P45_consists_of aat:300015050 ;  # Huile
    crm:P45_consists_of aat:300014078 ;  # Toile
    crm:P32_used_general_technique aat:300054216 ;  # Peinture
    crm:P52_has_current_owner votremusee:institution ;
    crm:P43_has_dimension votremusee:dimension/001 .
```

#### 3.3 Encoder l'événement de production

```turtle
votremusee:production/001 a crm:E12_Production ;
    crm:P14_carried_out_by votremusee:personne/artiste001 ;
    crm:P4_has_time-span votremusee:timespan/001 ;
    crm:P7_took_place_at votremusee:lieu/001 .

votremusee:personne/artiste001 a crm:E21_Person ;
    rdfs:label "Nom de l'artiste"@fr ;
    owl:sameAs wd:QXXXXX .  # URI Wikidata si disponible

votremusee:timespan/001 a crm:E52_Time-Span ;
    crm:P82a_begin_of_the_begin "1783-01-01"^^xsd:date ;
    crm:P82b_end_of_the_end "1783-12-31"^^xsd:date .

votremusee:lieu/001 a crm:E53_Place ;
    rdfs:label "Paris"@fr ;
    owl:sameAs wd:Q90 .  # Wikidata: Paris
```

#### 3.4 Encoder les dimensions

```turtle
votremusee:dimension/001 a crm:E54_Dimension ;
    crm:P90_has_value "150"^^xsd:decimal ;
    crm:P91_has_unit <http://vocab.getty.edu/aat/300379098> ;  # centimètres
    crm:P2_has_type <http://vocab.getty.edu/aat/300055644> .  # hauteur
```

#### 3.5 Encoder un événement d'acquisition

```turtle
votremusee:objet/001 crm:P24i_changed_ownership_through votremusee:acquisition/001 .

votremusee:acquisition/001 a crm:E8_Acquisition ;
    crm:P4_has_time-span votremusee:timespan/002 ;
    crm:P22_transferred_title_to votremusee:institution ;
    crm:P23_transferred_title_from votremusee:personne/donateur001 ;
    crm:P2_has_type <http://vocab.getty.edu/aat/300417642> .  # don

votremusee:timespan/002 a crm:E52_Time-Span ;
    crm:P82a_begin_of_the_begin "1920-03-15"^^xsd:date .
```

#### 3.6 Encoder une restauration (si applicable)

```turtle
votremusee:objet/001 crm:P31i_was_modified_by votremusee:restauration/001 .

votremusee:restauration/001 a crm:E11_Modification ;
    rdfs:label "Restauration du vernis"@fr ;
    crm:P14_carried_out_by votremusee:personne/restaurateur001 ;
    crm:P4_has_time-span votremusee:timespan/003 ;
    crm:P32_used_
...
# Le Web Sémantique et Linked Art

## Introduction au Web Sémantique

### Contexte et enjeux pour les institutions culturelles

Le web sémantique représente une évolution majeure dans la manière dont nous structurons et partageons l'information culturelle. Pour les musées, il s'agit de passer d'un **web de documents** à un **web de données**, permettant ainsi une interopérabilité accrue et de nouvelles possibilités de recherche et de découverte.

#### Les défis pour les institutions muséales

Les musées font face à plusieurs enjeux dans l'adoption du web sémantique :

- **Compétences techniques** : L'adoption de ces technologies nécessite des compétences spécifiques en modélisation de données et en technologies du web
- **Politiques d'ouverture** : Le partage des données suppose l'adoption de politiques d'ouverture adaptées
- **Autorité institutionnelle** : Dans un contexte distribué, il y a un changement d'échelle qui impacte la manière dont les institutions gèrent leurs collections
- **Interopérabilité** : Comment faire dialoguer des données hétérogènes provenant de multiples institutions?

---

## Architecture du Web : Fondements

### Les principes de base

Le web repose sur trois composants essentiels :

1. **HTTP** (HyperText Transfer Protocol) : Un protocole de communication
2. **URI/IRI** (Uniform/International Resource Identifier) : Un système d'identification des ressources
3. **HTML** : Un langage de représentation et d'hypertexte
...
RDF structure les descriptions en informations atomiques sous la forme : sujet-prédicat-objet. C'est un modèle de graphe où chaque assertion prend la forme d'un triplet.

### Structure d'un triplet RDF

```turtle
<http://example.org/artworks/mona-lisa> 
    <http://purl.org/dc/terms/creator> 
    <http://example.org/artists/leonardo-da-vinci> .
```

- **Sujet** : La Joconde (identifiée par une URI)
- **Prédicat** : a pour créateur
- **Objet** : Léonard de Vinci (identifié par une URI)
...
<http://musee.example.org/objet/123>
    a crm:E22_Human-Made_Object ;
    dc:title "Portrait de femme"@fr ;
    crm:P108i_was_produced_by <http://musee.example.org/production/123> ;
    crm:P45_consists_of <http://musee.example.org/materiau/huile> .
```

---

## CIDOC-CRM : Le Modèle Conceptuel de Référence

### Qu'est-ce que CIDOC-CRM?

Le CIDOC-CRM fournit des définitions et une structure formelle pour la description de concepts implicites et explicites et leurs relations utilisés dans la documentation du patrimoine culturel.

### Objectifs principaux

1. **Interopérabilité** : Permettre l'échange d'information entre sources hétérogènes
2. **Sémantique commune** : Fournir des définitions partagées et non ambiguës
3. **Perspective supra-institutionnelle** : Abstraire les contextes locaux spécifiques

### Classes principales du CIDOC-CRM

Le modèle distingue notamment :

- **E21_Person** : Personnes (artistes, collectionneurs, etc.)
- **E22_Human-Made_Object** : Objets créés par l'humain (œuvres d'art)
- **E53_Place** : Lieux géographiques
- **E52_Time-Span** : Périodes temporelles
- **E5_Event** : Événements (création, acquisition, exposition)
...
### Le rôle des vocabulaires dans le web sémantique

Les vocabulaires contrôlés permettent :
- De normaliser les termes de description
- D'assurer la cohérence des données
- De faciliter les recherches multilingues
- D'établir des relations sémantiques entre concepts

### SKOS : Simple Knowledge Organization System

SKOS est un vocabulaire RDF permettant de décrire des référentiels de type thésaurus.

#### Exemple de concept SKOS
...
<http://vocab.getty.edu/aat/300033618>
    ...
    skos:broader <http://vocab.getty.edu/aat/300033611> ;
    skos:definition "Œuvre réalisée avec des pigments et un liant"@fr .
```

### Thésaurus majeurs pour les musées

#### Getty Vocabularies
- **AAT** (Art & Architecture Thesaurus) : Terminologie pour l'art et l'architecture
- **ULAN** (Union List of Artist Names) : Référentiel d'artistes
- **TGN** (Thesaurus of Geographic Names) : Noms de lieux

#### Autres ressources importantes
- **IconClass** : Classification iconographique
...
## Applications Pratiques : American Art Collaborative

### Le projet AAC

L'American Art Collaborative réunit 14 musées américains majeurs pour publier leurs collections en Linked Open Data selon le modèle Linked Art.
...
- Et 10 autres institutions

### Résultats et bénéfices

1. **Découvrabilité accrue** : Les œuvres sont plus facilement trouvables
2. **Recherches trans-institutionnelles** : Les chercheurs peuvent interroger plusieurs collections simultanément
3. **Enrichissement mutuel** : Les institutions bénéficient des données des autres
4. **Nouveaux usages** : Visualisations, analyses, applications tierces

### Exemple d'application

Rechercher tous les portraits d'Abraham Lincoln dans les collections participantes, avec leurs provenances et leurs expositions historiques.

---

## Mise en Œuvre : De la Théorie à la Pratique

### Étapes de publication en Linked Art

#### 1. Audit des données existantes
- Évaluer la qualité et la complétude des données
- Identifier les champs à privilégier
- Repérer les vocabulaires déjà utilisés

#### 2. Mapping vers Linked Art
- Établir les correspondances entre vos champs et le modèle Linked Art
- Documenter les choix de modélisation
- Gérer les cas particuliers

#### 3. Alignement avec les vocabulaires
- Mapper vos termes vers Getty AAT, ULAN, etc.
- Utiliser des outils d'alignement (OpenRefine, etc.)
- Valider manuellement les alignements critiques

#### 4. Transformation des données
- Convertir les données au format JSON-LD
- Générer les URIs stables et persistants
- Valider contre le schéma Linked Art

#### 5. Publication
- Choisir une plateforme (SPARQL endpoint, dumps RDF, etc.)
- Documenter l'API et les points d'accès
- Adopter une licence ouverte (idéalement CC0 ou CC-BY)
...
### Avantages du web sémantique pour les musées

#### Pour les institutions
- **Visibilité accrue** : Présence dans les moteurs de recherche et agrégateurs
- **Collaborations facilitées** : Projets inter-institutionnels simplifiés
- **Pérennité** : Standards ouverts et durables
- **Innovation** : Nouvelles possibilités de valorisation

#### Pour les utilisateurs
- **Découverte enrichie** : Navigation par concepts et relations
- **Recherche multilingue** : Accès dans différentes langues
- **Contextualisation** : Liens avec ressources externes (Wikipedia, etc.)
- **Réutilisation** : Données exploitables pour recherche et création

### Défis à relever

1. **Ressources humaines** : Formation du personnel aux nouvelles compétences
2. **Infrastructure technique** : Investissement en outils et serveurs
3. **Qualité des données** : Nettoyage et enrichissement préalables nécessaires
4. **Maintenance** : Mise à jour continue des alignements et des données
5. **Gouvernance** : Définition de politiques claires d'ouverture

---

## Étude de Cas : British Museum

### Approche du British Museum

Le British Museum publie ses collections en RDF depuis plusieurs années, utilisant :
- **CIDOC-CRM** comme modèle de base
- **Thésauri Getty** pour les vocabulaires
- **SPARQL endpoint** public pour l'interrogation

### Exemple de requête

Trouver tous les objets égyptiens en bronze :
...
SELECT ?object ?label
WHERE {
  ?object crm:P45_consists_of <http://collection.britishmuseum.org/id/thesauri/material/bronze> ;
          bmo:PX_has_main_representation ?image ;
          rdfs:label ?label .
  ?object crm:P108i_was_produced_by ?production .
  ?production crm:P7_took_place_at ?place .
  ?place crm:P89_falls_within <http://collection.britishmuseum.org/id/place/Egypt>
}
LIMIT 100
```
...
- **+250% d'utilisation** des données de collection
- **Intégration** dans Wikipedia/Wikidata
...
- Programming Historian : "Intro to Linked Data"
- Europeana : Documentation sur EDM
- W3C : SPARQL tutorials
- Linked Art : Modèles et exemples

### Publications recommandées
- Bermès, E. (2013). *Le Web sémantique en bibliothèque*
- Van Holland, S. & Verborgh, R. (2014). *Linked Data for Libraries, Archives and Museums*
- Doerr, M. (2009). "Ontologies for Cultural Heritage"

---

## Conclusion

### L'avenir du web sémantique muséal

Le web sémantique et Linked Art représentent un changement de paradigme pour les musées :

1. **Passage de silos à un réseau** : Les collections ne sont plus isolées mais interconnectées
2. **Nouvelles formes de médiation** : Les données structurées permettent des expériences innovantes
3. **Recherche enrichie** : Les chercheurs accèdent à des corpus élargis et interrogeables finement
4. **Pérennité et résilience** : Les standards ouverts garantissent la durabilité

### Perspectives

- **Intelligence artificielle** : Enrichissement automatique et détection de liens
- **Visualisation** : Nouvelles interfaces de navigation dans les graphes
- **Personnalisation** : Expériences sur mesure basées sur les données sémantiques
- **Recherche fédérée** : Interrogation simultanée de milliers d'institutions

### Message clé

Le web sémantique transforme la manière dont nous pouvons découvrir, analyser et visualiser les contenus culturels et scientifiques. Pour les musées, adopter ces technologies n'est plus une option mais une nécessité pour rester pertinent dans l'écosystème numérique contemporain.
...
- Les choix stratégiques pour votre institution
- Les outils et ressources disponibles
- Les expériences d'autres musées

**Contact et ressources** :
- Documentation Linked Art : https://linked.art/
- Exemples de données : https://linked.art/api/1.0/
- Communauté LODLAM : https://www.lodlam.net/


# Le Web Sémantique et Linked Art
## Application pratique à la description des collections muséales
...
- Les vocabulaires contrôlés (thésaurus, SKOS)
- Les principes de normalisation terminologique
- L'importance des référentiels partagés

**Aujourd'hui**, nous allons voir comment ces vocabulaires s'intègrent dans un **modèle de données** pour créer des descriptions riches et interopérables de vos objets de collection.

### L'exercice d'aujourd'hui

Vous avez tenté de décrire un objet de collection en utilisant la documentation de Linked Art. 

**Objectif de ce cours** : 
- Comprendre les fondements théoriques de votre démarche pratique
- Éclairer les choix de modélisation que vous avez faits
...
## Architecture du Web : Les fondations

### Les trois piliers du web

#### 1. HTTP : Le protocole de communication
```
GET /objet/123 HTTP/1.1
Host: musee.example.org
Accept: application/json
```
- Permet de demander et recevoir des ressources
- Standard universel pour échanger des informations

#### 2. URI/IRI : Les identifiants uniques
```
https://musee.example.org/objet/joconde
https://vocab.getty.edu/aat/300033618
https://www.wikidata.org/wiki/Q762
```
- Chaque chose a une adresse unique sur le web
- Permet de référencer sans ambiguïté

#### 3. Format de données : La représentation
```json
{
...
## Le modèle RDF : Penser en triplets

### Le principe du triplet
...
En RDF :
```turtle
<http://musee.org/joconde> 
    <http://créé-par> 
    <http://musee.org/leonard> .
```

### Exemple concret : Votre objet

Si vous décrivez un tableau :

```turtle
<http://votremusee.org/objet/456>
    <http://a-pour-titre> "Nature morte aux pommes" ;
    <http://créé-par> <http://votremusee.org/artiste/cezanne> ;
    <http://date-création> "1895" ;
    <http://technique> <http://vocab.getty.edu/aat/300033618> .
```
...
```
        a pour titre
Objet ───────────────→ "Nature morte aux pommes"
  │
  ├─ créé par ────────→ Cézanne
  │
  ├─ date ────────────→ 1895
  │
  └─ technique ───────→ peinture à l'huile (AAT)
```

---

## CIDOC-CRM : Le modèle conceptuel

### Pourquoi CIDOC-CRM ?

CIDOC-CRM est le **modèle de référence** pour le patrimoine culturel, développé depuis 1996 et normalisé ISO.

#### Ses objectifs
1. **Interopérabilité** : Permettre l'échange entre institutions
2. **Richesse sémantique** : Capturer les nuances de la documentation muséale
3. **Flexibilité** : S'adapter à tous les types de collections

### Les concepts clés

#### Entités vs Événements

CIDOC-CRM distingue :
- **Les choses** (E22 Human-Made Object) : l'objet physique
- **Les événements** (E12 Production) : sa création
- **Les acteurs** (E21 Person) : les personnes impliquées
- **Les lieux** (E53 Place) : où cela s'est passé
- **Le temps** (E52 Time-Span) : quand cela s'est passé

#### Approche centrée sur les événements

Au lieu de :
```
Objet → créé par → Artiste
```

CIDOC-CRM dit :
```
Objet → a été produit par → [Événement de Production]
                                    ↓
                            réalisé par → Artiste
                            a eu lieu à → Lieu
                            dans la période → Date
```

---

## Linked Art : CIDOC-CRM simplifié

### Le problème de CIDOC-CRM

CIDOC-CRM est **très puissant** mais aussi **très complexe** :
- Plus de 90 classes
- Plus de 150 propriétés
- Nombreuses subtilités conceptuelles

Pour un conservateur ou un développeur, c'est intimidant !

### La solution Linked Art

Linked Art est un **profil d'application** de CIDOC-CRM :
- Sélectionne les parties les plus utiles
- Simplifie les patterns complexes
- Fournit des exemples concrets
- Utilise JSON-LD (facile à lire et manipuler)

#### Principe : 80% des besoins avec 20% de la complexité
...
## Retour sur votre exercice

### Les défis communs rencontrés

#### 1. "Où mettre l'information X ?"

**Exemple** : La date de création


**Pourquoi ?** La date est une propriété de l'événement de production, pas de l'objet.

---

### Les défis communs (suite)

#### 2. "Faut-il tout mettre en URI ?"

**Règle générale** :
- **URI** : Pour les concepts contrôlés (types, matériaux, personnes connues, lieux)
- **Littéral** : Pour les valeurs uniques (titres, descriptions, numéros)

**Exemple** :
```json
// ✅ Bon : Technique avec URI AAT
"technique": [
  {
...
// ❌ Éviter : Créer une URI pour chaque titre
"identified_by": [
  {
    "id": "https://musee.example.org/titre/123",
    "type": "Name"
  }
]
```
...
#### 3. "Comment gérer l'incertitude ?"

**Dates approximatives** :
```json
"timespan": {
  "type": "TimeSpan",
  "begin_of_the_begin": "1890-01-01",
  "end_of_the_end": "1895-12-31",
  "identified_by": [
    {
      "type": "Name",
      "content": "vers 1893"
...
"attributed_by": [
  {
    ...
    "classified_as": [
      {
        "id": "http://vocab.getty.edu/aat/300404114",
        "type": "Type",
        "_label": "attributed to"
      }
    ]
  }
]
...
#### 4. "Comment documenter mes sources ?"

Chaque assertion peut avoir une source :
...
"referred_to_by": [
  {
    ...
    "content": "Acquis de la collection Durand en 1985",
    "identified_by": [
      {
        "type": "Name",
        "content": "Source : Registre d'entrées, p. 234"
      }
    ]
  }
]
...
### Synthèse : Une notice complète

Voyons maintenant comment tous ces éléments s'assemblent dans une notice réelle.
...
```json
{
  ...
  "type": "HumanMadeObject",
  "_label": "Portrait de Lisa Gherardini, épouse de Francesco del Giocondo",
...
```json
    "technique": [
      {
        "id": "http://vocab.getty.edu/aat/300178684",
        "type": "Type",
        "_label": "oil painting"
      }
    ]
  ...
  "made_of": [
    {
      ...
      "type": "Material",
      "_label": "peinture à l'huile"
    },
    {
      "id": "http://vocab.getty.edu/aat/300014657",
      "type": "Material",
      "_label": "bois de peuplier"
    }
  ],
...
```json
  ...
  "current_owner": [
    {
      "id": "https://www.wikidata.org/wiki/Q19675",
      "type": "Group",
      "_label": "Musée du Louvre"
    }
  ],
  
  "current_location": {
    "id": "https://musee.louvre.fr/lieu/salle-711",
    "type": "Place",
    "_label": "Salle des États"
  },
```
...
```json
  "represents": [
    {
      "id": "https://www.wikidata.org/wiki/Q13428",
      "type": "Person",
      "_label": "Lisa Gherardini"
    }
  ],
  ...
  "representation": [
    {
      ...
      "type": "VisualItem",
      "_label": "Image numérique de La Joconde",
      "classified_as": [
        {
          "id": "http://vocab.getty.edu/aat/300215302",
          "type": "Type",
          "_label": "images numériques"
        }
      ]
    }
  ],
  ...
  "referred_to_by": [
    {
      ...
      "content": "Portrait d'une jeune femme florentine, probablement Lisa Gherardini, épouse du marchand Francesco del Giocondo. L'œuvre est célèbre pour l'expression énigmatique du sujet et la technique du sfumato."
    }
  ]
}
```
...
- Vérifier la structure avec le schéma Linked Art

#### 2. Validation sémantique
Posez-vous ces questions :
- ✅ Toutes les URIs sont-elles résolvables ?
- ✅ Les types correspondent-ils au modèle Linked Art ?
- ✅ Les vocabulaires utilisés sont-ils cohérents ?
- ✅ Les relations sont-elles dans le bon sens ?

#### 3. Validation humaine
- ✅ Un collègue peut-il comprendre votre modélisation ?
- ✅ L'information essentielle est-elle présente ?
- ✅ Les nuances importantes sont-elles capturées ?

---

### Bonnes pratiques

#### URIs stables et persistants

❌ À éviter :
```
http://musee.org/temp/objet123.html
http://192.168.1.1/collection/item?id=123
```

✅ Recommandé :
```
https://musee.example.org/objet/joconde
https://musee.example.org/collection/peinture/inv-779
```

#### Principe :
- Pas de numéros IP
- Pas de extensions de fichier (.html, .php)
- Pas de paramètres de requête (?id=)
- Structure logique et parlante

---

### Bonnes pratiques (suite)

#### Utilisation des vocabulaires

**Ordre de préférence** :

1. **Getty AAT** pour types, matériaux, techniques
   - Multilingue, bien maintenu, largement adopté

2. **Getty ULAN** pour les personnes (artistes)
   - Autorité reconnue, riche en biographie

3. **Getty TGN** pour les lieux
   - Hiérarchie géographique complète

4. **Wikidata** comme complément
   - Très complet mais parfois moins précis
   - Excellent pour les concepts manquants ailleurs

5. **Vocabulaires locaux** en dernier recours
   - Seulement si rien d'autre n'existe
...
**Toujours inclure `_label`** :
```json
{
  "id": "http://vocab.getty.edu/aat/300033618",
  "type": "Type",
  "_label": "paintings"  // ← Important pour la lisibilité
}
```

**Commentaires sur les choix complexes** :
```json
"referred_to_by": [
  {
    "type": "LinguisticObject",
    ...
    "content": "L'attribution à Rembrandt est contestée. Certains historiens suggèrent un élève de son atelier."
  }
]
```
...
### Pour travailler avec Linked Art
...
- **JSON Crack** : https://jsoncrack.com/
  - Visualise le JSON comme un graphe
- **JSON Editor Online** : https://jsoneditoronline.org/
  - Édition et validation

#### 3. Validateurs
- **Linked Art Validator** (en développement)
...
## SPARQL : Interroger vos données

### Introduction à SPARQL

Une fois vos données en Linked Art, vous pouvez les interroger avec **SPARQL**, le langage de requête pour RDF.
...
SELECT ?objet ?titre
WHERE {
  ?objet a crm:E22_Human-Made_Object ;
         crm:P102_has_title ?titre .
}
```

**Décomposition** :
- `PREFIX` : Déclare les espaces de noms (comme @context en JSON-LD)
- `SELECT` : Ce qu'on veut récupérer
- `WHERE` : Les conditions (patterns de triplets)
...
WHERE {
  ?objet a crm:E22_Human-Made_Object ;
         crm:P2_has_type aat:300033618 ;  # peinture
         crm:P102_has_title ?titre .
}
```
...
SELECT ?oeuvre ?titre ?date
WHERE {
  ?oeuvre crm:P108i_was_produced_by ?production .
  ?production crm:P14_carried_out_by <http://vocab.getty.edu/ulan/500010879> ; # Léonard
              crm:P4_has_time-span ?timespan .
  ?oeuvre crm:P102_has_title ?titre .
...
#### Compter les œuvres par technique

```sparql
PREFIX crm: <http://www.cidoc-crm.org/cidoc-crm/>

SELECT ?technique (COUNT(?oeuvre) AS ?nombre)
WHERE {
  ?oeuvre crm:P108i_was_produced_by ?production .
...
SELECT ?objet ?ancienProprio ?dateCession
WHERE {
  ?objet crm:P24i_changed_ownership_through ?acquisition .
  ?acquisition crm:P23_transferred_title_from ?ancienProprio ;
               crm:P4_has_time-span ?timespan .
  ?timespan crm:P82a_begin_of_the_begin ?dateCession .
  FILTER(?dateCession >= "1900-01-01"^^xsd:date)